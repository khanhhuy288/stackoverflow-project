...
with st.form("predictor_form"):
    ...
    with col_left:
        ...
        dev_type = st.selectbox("Developer Role", dev_types, index=None, placeholder="Select your role")
        employment = st.selectbox("Employment", employment_opts, index=None, placeholder="Select employment type")
        org_size = st.selectbox("Company Size", org_sizes, index=None, placeholder="Select company size")

    with col_right:
        work_exp = st.slider("Years of Professional Experience", 0, 50, 5)
        years_code = st.slider("Total Years Coding", 0, 50, 5)
        ...
        ed_level = st.selectbox("Education Level", ed_levels, index=None, placeholder="Select education level")
...
if submitted:
    ...
    m2.metric("Likely Range", f"${result['low']:,} – ${result['high']:,}")
    ...
    top_n = 15 if len(delta_df) > 15 else len(delta_df)
